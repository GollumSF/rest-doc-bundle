...
class TypeDiscoverPassTest extends AbstractCompilerPassTestCase {
	...
	public function testProcessNot() {
		...
		$serviceTag1->addTag(TypeDiscoverInterface::HANDLER_TAG, [ 'priority' => 20 ]);
		$this->setDefinition('serviceTag1', $serviceTag1);

		$serviceTag2 = new Definition();
		$serviceTag2->addTag(TypeDiscoverInterface::HANDLER_TAG);
		$this->setDefinition('serviceTag2', $serviceTag2);

		$serviceTag3 = new Definition();
		$serviceTag3->addTag(TypeDiscoverInterface::HANDLER_TAG, [ 'priority' => 10 ]);
		$this->setDefinition('serviceTag3', $serviceTag3);

		$serviceTag4 = new Definition();
		$serviceTag4->addTag(TypeDiscoverInterface::HANDLER_TAG, [ 'priority' => 20 ]);
		$this->setDefinition('serviceTag4', $serviceTag4);
		
		
		$service = new Definition();
		$this->setDefinition(TypeDiscoverInterface::class, $service);
	...
	public function testProcessNotDeclared() {
		...
		$container
			->expects($this->once())
			->method('has')
			->with(TypeDiscoverInterface::class)
			->willReturn(false);
		;
		...
		$compilerPass = new TypeDiscoverPass();
